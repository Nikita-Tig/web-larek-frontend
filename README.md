...
-	openProduct –– для эмита что был клик по каоточке.
...
3. Класс OrderModel
Хранит тип оплаты, адрес, email, номер телефона.
Конструктор принимает такие аргументы: events: IEvents.
Класс имеет такие методы:
...
Класс имеет такие методы:
- openBasket –– для открытия корзины.
...
Конструктор проверяет наличие элемента при помощи метода ensureElement и возвращает его как элемент
Класс имеет такие методы:
-	setSelectedPayment –– для вывода выбранного способа оплаьы.
...
Его методы:
- openModal –– при нажатии на карточку открывает popup.
- closeModal –– при нажатии на кнопку или за пределами popup, закрывает popup.
