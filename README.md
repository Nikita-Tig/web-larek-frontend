...
- validateInput –– проверяет введен ли корректно адрес, email или номер телефона. Если да, то сохраняет значения полей.
-	validatePaymentForm –– проверяет введены ли корректно данные в форме оплаты.
- validateContactsForm –– проверяет введены ли корректно данные в форме контактов.
- setTotalCost –– сохраняет переданную информации о стоимости товаров.
- setItems –– сохраняет id продуктов которые приобретаются.
- order –– записывает все сохраненные данные в объект.
...
-	setValidationError –– для вывода ошибок при заполнении полей.
...
interface IPayment {
    validationError: string; // значение ошибки.
}
...
validate:email –– событие исходящее при вводе чего либо в поле ввода электронной почты.
validate:phone –– событие исходящее при вводе чего либо в поле ввода телефона.
